...
func (s *Scanner) setSemanticModel(model []byte, expectedHash string) {
	...
	if detector != nil && !s.semanticThresholdSet {
...
func (s *Scanner) semanticAssessment(normalized string) *contracts.SemanticThreatAssessment {
	if s.semantic == nil {
		...
		if s.semanticThresholdBP > 0 {
			assessment.ThresholdBP = s.semanticThresholdBP
		}
		return &assessment
	}
...
func (d *SemanticDetector) Assess(input string, thresholdBP int) *contracts.SemanticThreatAssessment {
	...
	scoreWindow := func(window []string) {
		...
		for _, centroid := range d.model.Centroids {
			...
			// A score that had to be reduced to zero must not claim a nearest
			// class. Otherwise ordinary out-of-vocabulary input appears to
			// carry classifier confidence it never earned.
			if score == 0 {
				continue
			}
...
func (d *SemanticDetector) embedTokens(tokens []string) []int64 {
	...
	for _, token := range tokens {
		...
		bounded := semanticBoundedRunes(token)
		forEachPrefixSubwordRunes(bounded, accumulate)
		if matches == 0 {
			if len(bounded) < 8 {
				continue
			}
			trigramCount := int64(0)
			forEachCharTrigramRunes(bounded, func(subword string) {
				trigramCount++
				accumulate(subword)
			})
...
func semanticBoundedRunes(token string) []rune {
	bounded := make([]rune, 0, len(token)+2)
...
func forEachPrefixSubwordRunes(bounded []rune, yield func(string)) {
	for size := 5; size <= 8 && size <= len(bounded); size++ {
		yield("p:" + string(bounded[:size]))
	}
}

func forEachCharTrigramRunes(bounded []rune, yield func(string)) {
