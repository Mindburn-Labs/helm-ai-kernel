...
func (s *EffectDispositionService) ListReconciliationCandidates(ctx context.Context) (contracts.EffectReconciliationCandidates, error) {
	...
	return s.store.listEffectReconciliationCandidates(ctx, identity, s.releaseAuthorities, s.commandVerifier)
}
...
func (s *PostgresStore) listEffectReconciliationCandidates(
	...
	for _, reservation := range reservations {
		...
		chain, err := queryEffectDispositionRecordsForEffect(ctx, tx, identity.TenantID, identity.WorkspaceID, candidate.AdmissionID)
		if err != nil {
			return contracts.EffectReconciliationCandidates{}, err
		}
...
func queryCurrentEffectReconciliationReservations(ctx context.Context, tx *sql.Tx, identity ConsumerIdentity) ([]EffectReservationEvent, error) {
...
func effectReconciliationCandidateFromReservation(reservation EffectReservationEvent) (contracts.EffectReconciliationCandidate, bool, error) {
