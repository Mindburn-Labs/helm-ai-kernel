...
// runReportCmd implements `helm report` — generate compliance reports from receipt chains.
//
...
func buildComplianceReport(receipts []*contracts.Receipt, standard, period string) *ComplianceReport {
	report := &ComplianceReport{
		...
		GeneratedBy:      "helm report v" + displayVersion(),
...
func mapRequirements(standard string, report *ComplianceReport) []RequirementMapping {
	...
	default: // general
		return []RequirementMapping{
			...
			{Requirement: "Cryptographic Proof", Description: "Ed25519 signatures on all actions", Status: boolStatus(hasReceipts), Evidence: "Signed receipts verifiable via `helm verify`"},
		}
	}
}
