...
2. Update `CHANGELOG.md` with the `v0.5.9` user-visible delta.
...
5. Confirm `dist/release-assets/` contains CLI binaries, `SHA256SUMS.txt`,
   `sbom.json`, `v0.5.9.openvex.json`, `release-attestation.json`,
...
2. Replace the stale annotated `v0.5.9` tag only after the corrected release
   commit is on `main`.
...
5. Commit the post-publish docs update that changes “current public release”
   references to `v0.5.9` with the actual GitHub publish timestamp.
