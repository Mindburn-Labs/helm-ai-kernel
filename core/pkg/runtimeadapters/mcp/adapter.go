...
func (a *MCPAdapter) Intercept(ctx context.Context, req *runtimeadapters.AdaptedRequest) (*runtimeadapters.AdaptedResponse, error) {
	...
	// 1. Resolve PrincipalID to a VirtualEmployee via workforce registry
	// 2. Look up the tool in the MCP catalog
	// 3. Classify the effect type
	// 4. Call bridge.KernelBridge.Govern() for a verdict
	// 5. If ALLOW: execute via mcp.Gateway, create EFFECT node
	// 6. If DENY: return DenyReason with actionable guidance
	// 7. If ESCALATE: create InboxItem for approval
	//
	// For now, return the governance intercept proof as a placeholder.
	return &runtimeadapters.AdaptedResponse{
		Allowed:        false,
		DenyReason:     &runtimeadapters.DenyReason{Code: "NOT_IMPLEMENTED", Message: "MCP adapter governance bridge not yet wired", Actionable: "contact_admin"},
