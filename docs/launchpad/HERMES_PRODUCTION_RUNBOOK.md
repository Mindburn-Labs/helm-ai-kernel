...
HELM_LAUNCHKIT_ENABLED=1 helm-ai-kernel up hermes --target local --verify-only --json |
  jq -e '
...
HELM_LAUNCHKIT_ENABLED=1 helm-ai-kernel up hermes --target local --live --json | tee "$proof_json"
