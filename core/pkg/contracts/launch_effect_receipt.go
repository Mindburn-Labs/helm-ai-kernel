...
func VerifyLaunchEffectReceipt(receipt LaunchEffectReceipt, ctx LaunchEffectReceiptVerificationContext) error {
	...
	if ctx.VerifyEffectReservation == nil {
		return errors.New("launch effect receipt requires durable effect reservation verification")
	}
	if err := ctx.VerifyEffectReservation(receipt.EffectReservationRef, receipt.EffectReservationHash, receipt.DispatchAdmissionRef, receipt.DispatchAdmissionHash); err != nil {
		return fmt.Errorf("verify launch effect receipt durable effect reservation: %w", err)
	}
