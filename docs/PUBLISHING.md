...
| Python SDK | `helm-sdk` |
| Rust SDK | `helm-sdk` |
| Java SDK | Maven Central coordinate `io.github.mindburnlabs:helm-sdk:0.5.12` |
...
Release target: `v0.5.12`. The release is complete only after the tagged
...
<https://github.com/Mindburn-Labs/helm-ai-kernel/releases/tag/v0.5.12>.
...
AI Act high-risk reference pack. The `v0.5.10` release attaches a
`helm-ai-kernel.rb` formula asset for version `0.5.12` and publishes the same
...
For `v0.5.10`, use `SHA256SUMS.txt`, `sbom.json`, `v0.5.12.openvex.json`,
