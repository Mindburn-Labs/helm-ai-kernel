...
func (c *Compiler) Compile(req *CompileRequest) (*CompileResult, error) {
	...
	// Step 1: Decompose each raw step into PlanStep nodes.
	...
	// Step 2: Assign sandbox profiles based on risk class.
	...
	// Step 3: Propagate constraints from IntentTicket.
	policyConstraints := c.extractPolicyConstraints(req.Ticket)

	// Step 4: Attach truth annotations.
	truth := c.buildTruthAnnotation(req, allSteps)

	// Step 5: Build PlanSpec.
