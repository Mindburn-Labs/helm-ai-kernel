---
title: Public Docs Index
last_reviewed: 2026-07-18
---

# Public Docs Index

This source index groups public Kernel documentation by reader task. A
`source-public` publication status means the source is eligible for publication;
it does not claim that a live docs route or runtime is available.

## Learn

- [How HELM Works](HOW_HELM_WORKS.md)
- [HELM Proof Loop](PROOF_LOOP.md)
- [Local Proof Journey](LOCAL_PROOF_JOURNEY.md)

## Protect

- [Protect Local Coding Agents](quickstart/workstation-governance.md)
- [MCP](INTEGRATIONS/mcp.md)
- [AI Security Categories](AI_SECURITY_CATEGORIES.md)

## Build

- [Quickstart](QUICKSTART.md)
- [Developer Journey](DEVELOPER_JOURNEY.md)
- [Write Policies](architecture/policy-languages.md)
- [Run HELM On A Sealed Or Air-Gapped Host](guides/air-gap-appliance.md)
- [OpenAI Proxy](INTEGRATIONS/openai_baseurl.md)
- [OpenClaw](INTEGRATIONS/openclaw.md)
- [Hermes](INTEGRATIONS/hermes.md)
- [Agent Clients](INTEGRATIONS/agent-clients.md)
- [Framework Adapters](INTEGRATIONS/framework-adapters.md)
- [Tool Runtime Adapters](INTEGRATIONS/tool-runtime-adapters.md)

## Verify

- [Receipts](VERIFICATION.md)
- [Export and Verify EvidencePacks](guides/export-verify-evidencepacks.md)
- [Conformance](CONFORMANCE.md)
- [Quantum Posture](security/quantum-posture.md)

## Operate

- [Troubleshooting](TROUBLESHOOTING.md)
- [Implementation Partner Handoff](guides/implementation-partner.md)

## Reference

- [CLI](reference/cli.md)
- [Agent Risk Scan](reference/agent-risk-scan.md)
- [Skill Packs](reference/skill-packs.md)
- [HTTP API](reference/http-api.md)
- [SDKs](sdks/00_INDEX.md)
- [Examples Matrix](EXAMPLES.md)
