...
var decideCases = []struct {
	...
	{"regression-aws-s3-rm", "aws --profile production s3 rm s3://backups --recursive", "aws s3 rm"},
	{"regression-git-push-force", "git push --force origin main", "force option"},
	{"regression-git-push-short-force", "git push -fu origin main", "force option"},
	...
	{"regression-git-push-dynamic-remote", `git push "$REMOTE" main`, "unresolvable arguments"},
	{"regression-git-push-dynamic-global-value", `git -C "$REPO" push origin main`, "dynamic subcommand"},
	{"regression-terraform-dynamic-subcommand", `terraform "$COMMAND"`, "dynamic subcommand"},
	{"regression-aws-dynamic-service", `aws "$SERVICE" rm`, "dynamic service"},
	{"regression-database-dynamic-query", `psql -c "$QUERY"`, "unresolvable query"},
	{"regression-database-opaque-input", "psql -f migrations.sql", "input cannot be resolved"},
...
var passCases = []struct {
	...
	{"safe-git-push", "git push origin main"},
