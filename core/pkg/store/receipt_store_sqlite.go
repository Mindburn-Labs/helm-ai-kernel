...
func (s *SQLiteReceiptStore) migrate() error {
	...
	if err := s.ensureColumn("envelope", "TEXT"); err != nil {
		return err
	}
...
func insertSQLiteReceipt(ctx context.Context, execer sqlExecer, r *contracts.Receipt) error {
	query := `INSERT INTO receipts (
		receipt_id, decision_id, effect_id, external_reference_id, status, blob_hash, output_hash, timestamp, executor_id, metadata, signature, merkle_root, prev_hash, lamport_clock, args_hash, log_id, leaf_index, transparency, key_id, public_key_set, signature_profile, signature_algorithm, correlation_id, signature_version, envelope
	) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
	...
	// The envelope is the stored document; the columns beside it exist for
	// querying and indexing. A signature covers the whole receipt (HELM-303),
	// so reconstructing one field-by-field from columns silently drops every
	// field without a column and makes a valid receipt fail as if tampered.
	...
	_, err = execer.ExecContext(ctx, query,
		r.ReceiptID, r.DecisionID, r.EffectID, r.ExternalReferenceID, r.Status, r.BlobHash, r.OutputHash, timestamp, r.ExecutorID, string(metaJSON), r.Signature, r.MerkleRoot, r.PrevHash, r.LamportClock, r.ArgsHash, r.LogID, r.LeafIndex, nullableJSON(transparencyJSON), r.KeyID, nullableJSON(publicKeySetJSON), r.SignatureProfile, r.SignatureAlgorithm, r.CorrelationID, r.SignatureVersion, string(envelopeJSON),
	)
	if err != nil {
...
func (s *SQLiteReceiptStore) queryOne(ctx context.Context, query string, arg any) (*contracts.Receipt, error) {
	...
	err := row.Scan(&receiptID, &decisionID, &effectID, &externalID, &status, &blobHash, &outputHash, &timestamp, &executorID, &metaJSON, &signature, &merkleRoot, &prevHash, &lamport, &argsHash, &logID, &leafIndex, &transparency, &keyID, &publicKeySet, &sigProfile, &sigAlgorithm, &correlationID, &signatureVersion, &envelope)
	if err != nil {
		if err == sql.ErrNoRows {
	...
	return receiptFromSQLiteFields(receiptID, decisionID, effectID, externalID, status, blobHash, outputHash, timestamp, executorID, metaJSON, signature, merkleRoot, prevHash, lamport, argsHash, logID, leafIndex, transparency, keyID, publicKeySet, sigProfile, sigAlgorithm, correlationID, signatureVersion, envelope)
}
...
func scanSQLiteReceipt(scanner sqliteScanner) (*contracts.Receipt, error) {
	...
	if err := scanner.Scan(&receiptID, &decisionID, &effectID, &externalID, &status, &blobHash, &outputHash, &timestamp, &executorID, &metaJSON, &signature, &merkleRoot, &prevHash, &lamport, &argsHash, &logID, &leafIndex, &transparency, &keyID, &publicKeySet, &sigProfile, &sigAlgorithm, &correlationID, &signatureVersion, &envelope); err != nil {
		return nil, err
	}
	return receiptFromSQLiteFields(receiptID, decisionID, effectID, externalID, status, blobHash, outputHash, timestamp, executorID, metaJSON, signature, merkleRoot, prevHash, lamport, argsHash, logID, leafIndex, transparency, keyID, publicKeySet, sigProfile, sigAlgorithm, correlationID, signatureVersion, envelope)
}
...
func receiptFromSQLiteFields(receiptID, decisionID, effectID string, externalID sql.NullString, status, blobHash, outputHash, timestamp string, executorID, metaJSON, signature, merkleRoot, prevHash sql.NullString, lamport uint64, argsHash sql.NullString, logID sql.NullString, leafIndex uint64, transparency sql.NullString,
	keyID, publicKeySet, sigProfile, sigAlgorithm, correlationID, signatureVersion, envelope sql.NullString) (*contracts.Receipt, error) {
	// A receipt written with an envelope is restored from it verbatim. The
	// signature covers the whole receipt (HELM-303), and the column set is a
	// projection — rebuilding from columns drops every field that has none and
	// makes an untampered receipt fail verification. Rows written before the
	// envelope column existed fall through to the column path below and keep
	// verifying under their original (V4) preimage.
	if envelope.Valid && envelope.String != "" && envelope.String != "null" {
		var stored contracts.Receipt
		if err := json.Unmarshal([]byte(envelope.String), &stored); err != nil {
	...
	receipt := &contracts.Receipt{
		...
		// SignatureVersion selects the verify preimage; dropping it on load makes
		// a V5 signature verify against V4 and fail as if tampered (HELM-303).
