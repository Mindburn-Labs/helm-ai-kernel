...
func runIdentityCmd(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 || args[0] != "agents" {
		fmt.Fprintln(stderr, "Usage: helm-ai-kernel identity agents [--json]")
		return 2
	}
...
func runTelemetrySurfaceCmd(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 || args[0] != "otel-config" {
		fmt.Fprintln(stderr, "Usage: helm-ai-kernel telemetry otel-config [--json]")
		return 2
	}
...
func runCoexistenceSurfaceCmd(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 || args[0] != "manifest" {
		fmt.Fprintln(stderr, "Usage: helm-ai-kernel coexistence manifest [--json]")
		return 2
	}
...
func runIntegrateSurfaceCmd(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 || args[0] != "scaffold" {
		fmt.Fprintln(stderr, "Usage: helm-ai-kernel integrate scaffold --framework <name> [--json]")
		return 2
	}
...
func runMCPPending(args []string, stdout, stderr io.Writer) int {
	...
	pending := newLocalSurfaceRegistry().ListMCPServers()
	if *jsonOutput {
...
func mcpEscalationReason(record contracts.ExecutionBoundaryRecord) string {
	...
	if record.ReasonCode == contracts.ReasonSchemaViolation {
		return "MCP tool schema pin is missing or mismatched; credential verification remains required before dispatch"
	}
