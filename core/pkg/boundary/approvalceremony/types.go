...
// Package approvalceremony defines the Kernel lifecycle projection that turns
// a verified human quorum into a single-use approval grant. Durable ownership
// and atomic transition enforcement belong to the Control Plane.
...
// Record is the Kernel-defined approval lifecycle projection. Client
// submissions may supply assertions, but never this record or its state
// transitions.
type Record struct {
	...
	// ConsumedAudience identifies the execution boundary that atomically
	// redeemed the grant. It must equal the signed grant audience.
	ConsumedAudience string `json:"consumed_audience,omitempty"`
...
func (r Record) Validate() error {
	...
	case StateConsumed:
		if r.Grant == nil || r.ConsumedAt == nil || !validToken(r.ConsumedBy) || !validToken(r.ConsumedAudience) {
			return invalidRecord("consumed requires grant, consumed_at, consumed_by, and consumed_audience")
		}
		if r.ConsumedAudience != r.Grant.Audience {
			return invalidRecord("consumed_audience must equal the signed grant audience")
		}
	...
	case StateExpired, StateDenied:
		if r.ConsumedAt != nil || r.ConsumedBy != "" || r.ConsumedAudience != "" {
		...
		if r.Grant == nil && r.Challenge == nil && r.ExpiresAt != nil {
			return invalidRecord("terminal state without authority artifacts cannot have expires_at")
		}
		latestArtifactAt := r.CreatedAt
		if r.Challenge != nil && r.Challenge.IssuedAt.After(latestArtifactAt) {
			latestArtifactAt = r.Challenge.IssuedAt
		}
		if r.VerifiedRef != nil && r.VerifiedRef.VerifiedAt.After(latestArtifactAt) {
			latestArtifactAt = r.VerifiedRef.VerifiedAt
		}
		if r.Grant != nil && r.Grant.IssuedAt.After(latestArtifactAt) {
			latestArtifactAt = r.Grant.IssuedAt
		}
		if r.UpdatedAt.Before(latestArtifactAt) {
			return invalidRecord("terminal transition precedes committed authority artifacts")
		}
	...
	if r.State != StateConsumed && (r.ConsumedAt != nil || r.ConsumedBy != "" || r.ConsumedAudience != "") {
...
func validateVerifiedRef(challenge contracts.ApprovalChallenge, verified approvalverify.VerifiedApprovalRef) error {
	...
	signerSetHash, err := approvalverify.ComputeSignerSetHash(
		verified.ChallengeHash,
		verified.AuthoritySnapshotHash,
		verified.RequiredRole,
		verified.Signers,
	)
	if err != nil || signerSetHash != verified.SignerSetHash {
		return invalidRecord("verified_ref signer set integrity mismatch")
	}
