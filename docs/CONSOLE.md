...
HELM AI Kernel does not promise the HELM AI Enterprise Console experience. This page should describe the public inspection and source artifacts that the Console consumes: receipts, ProofGraph nodes, policy bundle metadata, verification output, and exported evidence. If a screen or workflow exists only in HELM AI Enterprise, link to authenticated customer documentation from the product site and keep anonymous kernel docs focused on the data contracts a developer can generate locally. The validation path is to create a receipt, inspect it through the CLI or export, and verify that the fields named here are present in the public schema.
