...
<!-- quantum_posture: this page compares classical Ed25519 evidence signing and adds no post-quantum cryptographic control. -->
...
| Kill switch / HITL | Kill switch + human-in-the-loop approvals | Global freeze (`helm-ai-kernel freeze`) and the scoped emergency-stop fence; approval ceremonies (`helm-ai-kernel approvals`) recorded separately; quarantine registry (default-deny, and nothing approves a server yet). An `ESCALATE` verdict stops the call but is not routed to an approver: no path links it to a ceremony |
