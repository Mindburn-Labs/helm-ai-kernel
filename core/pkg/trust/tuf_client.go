...
type TUFClient struct {
	...
	// httpClient bounds metadata fetch latency.
...
func NewTUFClient(config TUFClientConfig) (*TUFClient, error) {
	...
	client := &TUFClient{
		...
		remoteURL:  strings.TrimRight(config.RemoteURL, "/"),
...
func (c *TUFClient) fetchAndVerify(role TUFRole) (*SignedRole, error) {
	endpoint := c.remoteURL + "/" + string(role) + ".json"
	resp, err := c.httpClient.Get(endpoint)
	if err != nil {
		return nil, fmt.Errorf("failed to fetch %s metadata: %w", role, err)
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		body, _ := io.ReadAll(io.LimitReader(resp.Body, 4096))
		return nil, fmt.Errorf("fetch %s metadata returned %s: %s", role, resp.Status, strings.TrimSpace(string(body)))
	}

	var signed SignedRole
	if err := json.NewDecoder(resp.Body).Decode(&signed); err != nil {
		return nil, fmt.Errorf("failed to decode %s metadata: %w", role, err)
	}
	if len(signed.Signed) == 0 {
		return nil, fmt.Errorf("%s metadata missing signed payload", role)
	}
	if len(signed.Signatures) == 0 {
		return nil, fmt.Errorf("%s metadata missing signatures", role)
	}

	var meta RoleMetadata
	if err := json.Unmarshal(signed.Signed, &meta); err != nil {
		return nil, fmt.Errorf("failed to parse %s metadata: %w", role, err)
	}
	if meta.Type != "" &&
		meta.Type != string(role) &&
		meta.Type != string(role)+".json" &&
		!strings.HasSuffix(meta.Type, "/"+string(role)) {
		return nil, fmt.Errorf("metadata type %q does not match role %q", meta.Type, role)
	}
	if len(c.rootKeys) == 0 {
