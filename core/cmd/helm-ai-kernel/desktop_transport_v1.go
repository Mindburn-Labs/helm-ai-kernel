...
const (
	...
	desktopTransportV1HandoffPrefix   = "helm-desktop-transport-v1-handoff"
)

// desktopTransportV1 is the opt-in direct-child contract used by the packaged
// Desktop. Its key and nonce are consumed from the environment before runtime
// setup can spawn a subprocess.
...
func desktopTransportV1ForOptions(opts serverOptions) (*desktopTransportV1, error) {
	...
	return takeDesktopTransportV1()
}

func takeDesktopTransportV1() (*desktopTransportV1, error) {
	enabled := os.Getenv(desktopTransportV1EnabledEnv)
	key := os.Getenv(desktopTransportV1KeyEnv)
	nonce := os.Getenv(desktopTransportV1NonceEnv)
	for _, name := range []string{desktopTransportV1EnabledEnv, desktopTransportV1KeyEnv, desktopTransportV1NonceEnv} {
		_ = os.Unsetenv(name)
	}
	if enabled != "1" {
		return nil, fmt.Errorf("%s must be exactly \"1\"", desktopTransportV1EnabledEnv)
	}
	if !desktopTransportV1IsLowerHex(key) {
		return nil, fmt.Errorf("%s must be %d lowercase hexadecimal characters", desktopTransportV1KeyEnv, desktopTransportV1SecretLength)
	}
	if !desktopTransportV1IsLowerHex(nonce) {
		return nil, fmt.Errorf("%s must be %d lowercase hexadecimal characters", desktopTransportV1NonceEnv, desktopTransportV1SecretLength)
	}
...
func (t *desktopTransportV1) hmac(message string) string {
	// The Desktop contract uses the canonical hex text as HMAC key bytes.
...
func registerDesktopTransportV1ProofRoute(mux *http.ServeMux, transport *desktopTransportV1, origin string) {
	...
	mux.HandleFunc(desktopTransportV1ProofPath, func(w http.ResponseWriter, r *http.Request) {
		...
		challenges := r.Header.Values(desktopTransportV1ChallengeHeader)
		if !requestFromLoopback(r) || len(r.Header.Values("Authorization")) != 0 || len(challenges) != 1 || !desktopTransportV1IsLowerHex(challenges[0]) {
			w.WriteHeader(http.StatusNotFound)
			return
		}
		w.Header().Set("Cache-Control", "no-store")
		w.Header().Set(desktopTransportV1ProofHeader, transport.handoffProof(origin, challenges[0]))
...
func desktopTransportV1SuppressesAuxiliaryHealth(transport *desktopTransportV1, healthPort int, metricsEnabled bool) (bool, error) {
	...
	if metricsEnabled {
		return false, fmt.Errorf("HELM_METRICS_ENABLED cannot be enabled when HELM_HEALTH_PORT=0 in desktop transport v1")
	}
	return true, nil
}
