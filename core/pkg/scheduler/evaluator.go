...
func NextFireTime(spec ScheduleSpec, after time.Time) (time.Time, error) {
	...
	case ScheduleRRULE:
		return nextRRULEFire(spec, after)
	default:
...
func nextRRULEFire(spec ScheduleSpec, after time.Time) (time.Time, error) {
	rule, err := parseRRULE(spec.Expression)
	if err != nil {
		return time.Time{}, fmt.Errorf("nextRRULEFire: %w", err)
	}
	loc, err := loadLocation(spec.Timezone)
	if err != nil {
		return time.Time{}, fmt.Errorf("nextRRULEFire: %w", err)
	...
	const maxIterations = 1_052_000 // two years of minutes
	for i := 0; i < maxIterations; i++ {
		if rruleMatches(rule, t) {
			return t, nil
		}
		t = t.Add(time.Minute)
	}
	return time.Time{}, fmt.Errorf("nextRRULEFire: no fire time found within search window for expression %q", spec.Expression)
}

func rruleMatches(rule *RRULESchedule, t time.Time) bool {
	if len(rule.ByHour) > 0 {
		if !contains(rule.ByHour, t.Hour()) {
			return false
		}
	} else if t.Hour() != 0 {
		return false
	...
	if len(rule.ByMinute) > 0 {
		if !contains(rule.ByMinute, t.Minute()) {
			return false
		}
	} else if t.Minute() != 0 {
		return false
	...
	case "HOURLY":
		hours := int(t.UTC().Sub(time.Unix(0, 0)) / time.Hour)
		return hours%rule.Interval == 0
	case "DAILY":
		days := int(t.UTC().Sub(time.Unix(0, 0)) / (24 * time.Hour))
		return days%rule.Interval == 0
	case "WEEKLY":
		if len(rule.ByDay) > 0 && !contains(rule.ByDay, int(t.Weekday())) {
			return false
		}
		weeks := int(t.UTC().Sub(time.Unix(0, 0)) / (7 * 24 * time.Hour))
