...
// runInitCmd implements `helm-ai-kernel init` — project scaffolding for HELM governance.
//
// Generates:
...
func runScaffoldCmd(args []string, stdout, stderr io.Writer) int {
	...
	fmt.Fprintln(stdout, "    2. Run `helm-ai-kernel dev` to start governance in dev mode")
	fmt.Fprintln(stdout, "    3. Connect your agent via MCP: `helm-ai-kernel mcp print-config --client <your-client>`")
	fmt.Fprintln(stdout)
...
// runDevCmd implements `helm-ai-kernel dev` — start HELM in development mode.
//
...
func runDevCmd(args []string, stdout, stderr io.Writer) int {
	...
	if _, err := os.Stat(config); os.IsNotExist(err) {
		...
		fmt.Fprintln(stderr, "Run `helm-ai-kernel init` first to scaffold your project.")
		return 2
	}
...
# Generated by helm-ai-kernel init v{{.Version}}
...
const defaultCELPolicy = `// HELM Default Policy — Generated by helm-ai-kernel init
//
...
2. Connect via MCP:
` + "```bash" + `
helm-ai-kernel mcp print-config --client <your-client>
` + "```" + `
...
## Verify Receipts
` + "```bash" + `
helm-ai-kernel verify <evidence-pack.tar.gz>
` + "```" + `
`,
