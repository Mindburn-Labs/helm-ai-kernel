...
// trackingDispatcher is a test double that flags whether it was ever called.
// If the firewall's allowlist check is bypassed, this records the leak.
type trackingDispatcher struct {
	dispatched bool
}

func (s *trackingDispatcher) Dispatch(_ context.Context, _ string, _ map[string]any) (any, error) {
	s.dispatched = true
