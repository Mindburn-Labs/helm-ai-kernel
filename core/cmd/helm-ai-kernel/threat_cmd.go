...
// runThreatCmd handles `helm-ai-kernel threat scan` and `helm-ai-kernel threat test` subcommands.
func runThreatCmd(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "Usage: helm-ai-kernel threat <scan|test> [flags]")
		return 2
	}
