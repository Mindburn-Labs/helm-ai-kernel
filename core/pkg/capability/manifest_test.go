...
func TestManifestValidate_Rejects(t *testing.T) {
	cases := []struct {
		...
		{"missing binding ref", func(m *Manifest) { m.Binding.Ref = "" }, "binding.ref"},
		{"bad binding kind", func(m *Manifest) { m.Binding.Kind = "browser_tab" }, "binding.kind"},
		...
		{"rollback not required", func(m *Manifest) { m.Rollback.Required = false }, "rollback.required"},
		...
		{"bad model tier", func(m *Manifest) { m.Routing.MinModelTier = "unbounded" }, "routing.min_model_tier"},
		{"bad source pack hash", func(m *Manifest) { m.Metadata.SourcePackHash = "sha256:bad" }, "metadata.source_pack_hash"},
		{"bad certification time", func(m *Manifest) { m.Metadata.CertifiedAt = "tomorrow" }, "metadata.certified_at"},
	}
