...
if [ -n "${HELM_CONSOLE_LOCAL_SIDECAR_DIR:-}" ]; then
    ...
    python3 "$SNAPSHOT_ROOT/scripts/release/console_local_sidecar.py" stage \
        --input-dir "$HELM_CONSOLE_LOCAL_SIDECAR_DIR" \
        --output-dir "$ASSETS_DIR" \
        --pins "$SNAPSHOT_ROOT/release/console-local-sidecar-pins.json" \
        --kernel-release "$TAG" \
        --expected-manifest-sha256 "$CONSOLE_LOCAL_SIDECAR_MANIFEST_SHA256"
    python3 "$SNAPSHOT_ROOT/scripts/release/console_local_sidecar.py" layout \
...
HELM_RELEASE_SOURCE_COMMIT="$SOURCE_COMMIT" \
    ...
    python3 - "$SNAPSHOT_ROOT" "$ASSETS_DIR" "$TAG" "${CONSOLE_LOCAL_SIDECAR_MANIFEST_SHA256:-}" <<'PY'
import hashlib
import json
import os
import pathlib
...
actual_openapi_blob = git_text("rev-parse", "--verify", f"{source_commit}:{api_contract['source_path']}")
