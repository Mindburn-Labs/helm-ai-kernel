...
func compileServePolicySnapshot(ctx context.Context, head policyreconcile.PolicyHead, bundle []byte) (*policyreconcile.EffectivePolicySnapshot, error) {
	...
	if runtimePolicy.Graph != nil {
		// Activation refuses a policy version with a rule that does not
		// compile, instead of installing it and denying that action later.
		if _, err := authority.Compile(runtimePolicy.Graph); err != nil {
			return nil, err
		}
	}
