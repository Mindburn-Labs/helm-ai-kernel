...
| Go SDK | `github.com/Mindburn-Labs/helm-ai-kernel/sdk/go@v0.5.15`; publish with the subdirectory tag `sdk/go/v0.5.14` |
...
Release target: `v0.5.15`. The release is complete only after the tagged
...
The `v0.5.14` release attaches a
`helm-ai-kernel.rb` formula asset for version `0.5.15` and publishes the same
...
For `v0.5.14`, use `SHA256SUMS.txt`, `sbom.json`, `v0.5.15.openvex.json`,
