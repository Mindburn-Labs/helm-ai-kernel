...
// runIncidentCmd implements `helm-ai-kernel incident` — incident registry management.
func runIncidentCmd(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "Usage: helm-ai-kernel incident <list|show|ack|create> [flags]")
		fmt.Fprintln(stderr, "")
	...
	case "--help", "-h":
		fmt.Fprintln(stdout, "Usage: helm-ai-kernel incident <list|show|ack|create> [flags]")
		return 0
	default:
...
// runMaintenanceCmd implements `helm-ai-kernel run maintenance` — governed self-fixing.
...
// runBriefCmd implements `helm-ai-kernel brief daily` — human-readable system summary.
func runBriefCmd(args []string, stdout, stderr io.Writer) int {
	...
	switch args[0] {
	case "--help", "-h":
		fmt.Fprintln(stdout, "Usage: helm-ai-kernel brief daily")
