...
func TestMemoryService_IngestReturnsError(t *testing.T) {
	...
	if err == nil {
		t.Error("expected error from stub ingestion")
	}
}
...
func TestInMemoryDecisionStore_CreateDuplicate(t *testing.T) {
	...
	dr := &contracts_stub_decision{RequestID: "d-1"}
...
// stub to avoid importing contracts
type contracts_stub_decision struct {
	RequestID string
}
