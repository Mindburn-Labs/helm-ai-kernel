...
func TestEffectReconciliationCandidatesValidate(t *testing.T) {
	...
	for name, mutate := range map[string]func(*EffectReconciliationCandidates){
		...
		"first sequence has prior receipt": func(p *EffectReconciliationCandidates) { p.Candidates[0].NextDispositionSequence = 1 },
		"duplicate admission":              func(p *EffectReconciliationCandidates) { p.Candidates = append(p.Candidates, p.Candidates[0]) },
	} {
