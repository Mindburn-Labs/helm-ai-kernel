...
class TestGeneratedTypes:
    ...
    def test_boundary_status_rejects_unknown_status(self) -> None:
        with pytest.raises(ValueError, match="must be one of enum values"):
            BoundaryStatus.from_dict({**BOUNDARY_STATUS_DATA, "status": "unknown"})
...
class TestExecutionBoundarySurfaces:
    ...
    @patch("helm_sdk.client.httpx.Client")
    def test_verifier_grade_routes(self, mock_client_cls: MagicMock) -> None:
        mock_client = mock_client_cls.return_value
        mock_client.get.return_value = mock_response(200, {
            "manifest_id": "env1",
