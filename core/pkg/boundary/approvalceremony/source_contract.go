...
// authenticated authority registry. The snapshot hash seals every key and
// scope before the data can enter approvalverify.TrustStore.
...
// and commits the exact source authority consumed by quorum verification.
...
// TrustStore re-seals and exact-compares the source commitment before exposing
// any key material to quorum verification.
func (snapshot AuthoritySnapshot) TrustStore() (approvalverify.TrustStore, error) {
	...
	if snapshot.AuthoritySnapshotHash == "" || sealed.AuthoritySnapshotHash != snapshot.AuthoritySnapshotHash {
		return approvalverify.TrustStore{}, errors.New("approval authority snapshot hash mismatch")
	}
