...
<!-- quantum_posture: this review oracle describes credential-verified dispatch
admission and receipt evidence, but it introduces no cryptographic algorithm
...
- MCP unknown servers and tools remain quarantined until a credential-verified
  durable dispatch admission is bound. Side-effect tools require the same
  admission plus durable effect-reservation enforcement.
...
| Unknown MCP server or tool | `ESCALATE` | `ERR_MCP_SERVER_QUARANTINED` | MCP quarantine decision and no side-effect dispatch until a credential-verified durable dispatch admission is bound. |
