...
(cd "$ROOT/core" && "$GO_BIN" test ./cmd/helm -run 'TestOpenAPIPathsAreRegisteredByHelmServeRuntime|TestRuntimeRouteRegistryMatchesOpenAPI|TestProtectedPublicRoutesDeclareOpenAPISecurity|TestProtectedRuntimeHandlersAreDeclaredInRouteRegistry' -count=1)
...
grep -q 'RuntimeRouteSpecs' "$ROOT/core/cmd/helm/route_registry.go" \
