...
// stubAdapter satisfies the channels.Adapter interface for testing.
type stubAdapter struct {
	kind channels.ChannelKind
}

func (s *stubAdapter) Kind() channels.ChannelKind { return s.kind }
func (s *stubAdapter) NormalizeInbound(_ context.Context, _ []byte) (channels.ChannelEnvelope, error) {
	return channels.ChannelEnvelope{}, nil
}
func (s *stubAdapter) Send(_ context.Context, _, _ string, _ channels.OutboundMessage) error {
	return nil
}
func (s *stubAdapter) Health(_ context.Context) error { return nil }

func TestAdapterRegistry_RegisterAndGet(t *testing.T) {
	reg := channels.NewAdapterRegistry()
	adapter := &stubAdapter{kind: channels.ChannelSlack}
...
func TestAdapterRegistry_List(t *testing.T) {
	reg := channels.NewAdapterRegistry()
	_ = reg.Register(&stubAdapter{kind: channels.ChannelSlack})
	_ = reg.Register(&stubAdapter{kind: channels.ChannelTelegram})
