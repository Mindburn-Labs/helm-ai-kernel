...
`VERSION` currently declares `0.7.2`. That is the source baseline, not a
published-release claim. At the last release-record check (2026-07-12), the
latest public GitHub Release was `v0.5.18` (published 2026-06-19). Do not
describe `0.7.2`, or any later source version, as publicly released until its
tag, assets, attestations, and verification evidence are published.
...
```bash
make prepare-version VERSION=<version>
make vex
```

2. Update `CHANGELOG.md` with the `v<version>` user-visible delta.
