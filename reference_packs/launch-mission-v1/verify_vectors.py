...
def verify_receipt(index, receipt_override=None):
    ...
    expected_reservation_ref = f"effect-reservation:{receipt['dispatch_admission_ref']}:2"
    if receipt.get("effect_reservation_ref") != expected_reservation_ref:
        raise VectorError("binding_mismatch", "receipt durable effect reservation differs from its dispatch admission")
