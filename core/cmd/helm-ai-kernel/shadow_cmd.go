...
// runShadowCmd implements `helm-ai-kernel shadow` — shadow-AI discovery scanner.
//
// Exit codes:
//
...
func runShadowCmd(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "Usage: helm-ai-kernel shadow <scan> [flags]")
		fmt.Fprintln(stderr, "")
	...
	case "--help", "-h":
		fmt.Fprintln(stdout, "Usage: helm-ai-kernel shadow <scan> [flags]")
		fmt.Fprintln(stdout, "")
