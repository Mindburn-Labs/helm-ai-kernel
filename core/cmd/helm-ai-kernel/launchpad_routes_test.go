...
func TestLaunchpadMCPApprovalFailsClosedWithoutVerifier(t *testing.T) {
	for name, payload := range map[string]string{
		"plausible scoped approval": `{
			"server_id":"srv-1",
		...
		"wildcard tool": `{"server_id":"srv-1","tools":["*"],"ttl":"15m","reason":"reviewed"}`,
		"malformed ttl": `{"server_id":"srv-1","tools":["write"],"ttl":"never","reason":"reviewed"}`,
	} {
		t.Run(name, func(t *testing.T) {
			...
			if rec.Code != http.StatusServiceUnavailable {
				t.Fatalf("expected unavailable verifier, got %d body=%s", rec.Code, rec.Body.String())
			}
			var problem struct {
			...
			if problem.Title != "MCP approval verification unavailable" || !strings.Contains(problem.Detail, "verification unavailable") {
				t.Fatalf("unexpected fail-closed response: %+v", problem)
			}
		})
	}
}
