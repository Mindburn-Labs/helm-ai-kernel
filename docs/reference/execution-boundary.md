...
| MCP quarantine and authorization | `helm-ai-kernel mcp scan`, `helm-ai-kernel mcp wrap`, `helm-ai-kernel mcp proof`, `helm-ai-kernel mcp list`, `helm-ai-kernel mcp get`, `helm-ai-kernel mcp quarantine`, `helm-ai-kernel mcp pending`, `helm-ai-kernel mcp approve` (rejects opaque local authority), `helm-ai-kernel mcp revoke`, `helm-ai-kernel mcp auth-profile`, `helm-ai-kernel mcp authorize-call` | `/api/v1/mcp/*`, `/.well-known/oauth-protected-resource/mcp` | Pre-dispatch MCP firewall state, no-dispatch proof bundles, and OAuth/profile bindings. Local opaque approval is rejected; bounded dispatch requires a credential-verified durable dispatch admission plus runtime checks. |
...
| Approvals and budgets | `helm-ai-kernel approvals *`, `helm-ai-kernel budget list`, `helm-ai-kernel budget set` | `/api/v1/approvals`, `/api/v1/budgets` | Local approval ceremonies and spend/tool/egress ceilings. The ceilings are recorded, not enforced: no shipped Guardian draws them down (`controls.yaml` CTL-011). |
