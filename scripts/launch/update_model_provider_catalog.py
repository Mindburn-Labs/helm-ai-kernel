...
def render_helper(catalog: dict) -> str:
    lines = [
        ...
        "",
        "# Egress readiness is proven by the provider probe below. Kubernetes",
        "# transparent egress carries no HTTP(S)_PROXY env, and local-container",
        "# proxy failures surface as curl HTTP 000 instead of a preflight env check.",
        "",
