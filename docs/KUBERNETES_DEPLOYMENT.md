...
| `image.tag` | chart `appVersion` | The source target is `v0.8.4` from `Chart.yaml`. It is not a published image until the tag-driven release workflow publishes and verifies it; use only a source-owned image tag (or local override) you can verify. |
