...
| F-14 | `.golangci.yml` never executed; gosec absent | partial | HELM-745 s4: `gosec` (golangci-lint, gosec only, core and sdk/go), `secrets` (gitleaks with the current key formats), `govulncheck` (all 13 Go modules) and `pip-audit` (Python SDK runtime requirements) now block in the `pr`, `merge` and `release` profiles, each behind a positive control (`scripts/ci/security_gates.sh`). The 544 gosec (taint rules G701–G706 excluded as nondeterministic) and 50 gitleaks findings present at that point are frozen in `scripts/ci/gosec-allowlist.txt` and `scripts/ci/gitleaks-allowlist.txt`, which can only shrink; triaging them remains open. The other golangci-lint linters in `.golangci.yml` still do not run in CI. |
...
when those fields were never stored. A causal append that encounters their
blank persisted `chain_hash` now fails closed and requires a new signed session.
Durable stores preflight that known state before external dispatch; their final
