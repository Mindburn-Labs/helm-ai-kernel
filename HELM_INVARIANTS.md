...
43 controls: 19 enforced, 12 observed-only, 12 unmanaged (11 of them retired invariants).
...
| CTL-018 | Mandates only narrow on delegation | observed-only | Library only. No shipped binary imports core/pkg/kernel/authority/authorityrows (a declared library root in scripts/ci/dead-packages-allowlist.txt), and admission does not read the authority rows yet. The HELM-751 admission transaction is the first caller. Until it lands, the Postgres tests prove the library. |
