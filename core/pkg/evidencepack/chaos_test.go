...
// empty manifest cannot be successfully verified — a zero-entry manifest
// with no content-addressed claims is not a valid evidence pack and must
// be rejected, not silently pass.
func TestChaos_evidence_empty_manifest_rejects_verification(t *testing.T) {
	m := &Manifest{} // intentionally empty
	if err := m.Validate(); err == nil {
		t.Fatal("CHAOS INVARIANT BROKEN: empty manifest passed Validate — " +
			"zero-entry manifests must not be considered valid evidence")
	} else if !strings.Contains(strings.ToLower(err.Error()), "empty") &&
		...
		t.Fatalf("expected error to indicate missing/invalid content, got: %v", err)
	}
}
