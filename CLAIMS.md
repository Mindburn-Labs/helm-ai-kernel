...
- Claim evidence: `core/cmd/helm-ai-kernel`, `core/pkg/guardian`, `core/pkg/receipt`, `core/pkg/evidence/externalhost`, `core/pkg/correlation/hostaction`, `core/pkg/verifier/externalreceipt`, `core/pkg/boundary/profile`, `protocols/json-schemas/boundary`, `reference_packs/boundary-profile-v1`, `reference_packs/update-bundle-v1`, `deploy/appliance`, `docs/deployment/boundary-enforcement-profile.md`, `api/openapi`, `Dockerfile`, `charts/helm-ai-kernel`, `README.md`.
- Tests that prove each claim: `go test ./...`, API contract checks, demo receipt smoke tests after API routes are enabled, `make verify-boundary-profile-vectors`, `make verify-update-bundle-vectors` (Go + independent Python parity incl. fail-closed negative vectors).
