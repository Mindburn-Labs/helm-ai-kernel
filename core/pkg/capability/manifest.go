...
func (m *Manifest) Validate() error {
	...
	if m.Binding.Kind == "" || m.Binding.Ref == "" {
		return fmt.Errorf("binding.kind and binding.ref are required")
	}
	...
	// reversibility-classes.md rule 1 ("no plan, no dispatch") enforced at
	// registration: reversible non-read-only capabilities must name a plan.
	if m.Reversibility != ReversibilityNone &&
		m.EffectClass != EffectReadOnly &&
		m.Rollback.Required && m.Rollback.PlanRef == "" {
