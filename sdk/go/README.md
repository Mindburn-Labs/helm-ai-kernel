...
After the `sdk/go/v0.7.3` subdirectory tag is published and verified:

```bash
go get github.com/Mindburn-Labs/helm-ai-kernel/sdk/go@v0.7.3
```

Version truth is the repository `VERSION` file (`0.7.3` for this source target).
Tagged Go module releases use the subdirectory tag form `sdk/go/v0.7.3`; this
...
`0.7.3` is the release-hardening patch with the public HTTP client surface, conformance entrypoints, and evidence verification helpers.
