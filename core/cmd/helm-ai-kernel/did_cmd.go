// did_cmd.go — `helm-ai-kernel did` CLI surface.
//
// Subcommands:
//
//	helm-ai-kernel did resolve <did>          Resolve a DID and print its DID Document.
//	helm-ai-kernel did verify <vc.json>       Verify a Verifiable Credential against its issuer DID.
//	helm-ai-kernel did list                   Show DIDs in the local DID keystore.
//	helm-ai-kernel did rotate [--key=<id>]    Rotate the local DID's signing key.
//
...
import (
	...
	"github.com/Mindburn-Labs/helm-ai-kernel/core/pkg/identity/did/method/jwk"
	"github.com/Mindburn-Labs/helm-ai-kernel/core/pkg/identity/did/method/key"
	"github.com/Mindburn-Labs/helm-ai-kernel/core/pkg/identity/did/method/plc"
	"github.com/Mindburn-Labs/helm-ai-kernel/core/pkg/identity/did/method/web"
...
func runDIDCmd(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "Usage: helm-ai-kernel did <resolve|verify|list|rotate> [args]")
		fmt.Fprintln(stderr, "")
...
func runDIDResolve(args []string, stdout, stderr io.Writer) int {
	...
	if cmd.NArg() < 1 {
		fmt.Fprintln(stderr, "Usage: helm-ai-kernel did resolve <did>")
		return 2
	}
	target := cmd.Arg(0)
...
func runDIDVerify(args []string, stdout, stderr io.Writer) int {
	...
	if cmd.NArg() < 1 {
		fmt.Fprintln(stderr, "Usage: helm-ai-kernel did verify <vc-path>")
		return 2
	}
	path := cmd.Arg(0)
...
func runDIDList(args []string, stdout, stderr io.Writer) int {
	...
	if len(ks.Entries) == 0 {
		fmt.Fprintln(stdout, "No DIDs registered. Run `helm-ai-kernel did rotate --key=<id>` to mint one.")
		return 0
	}
