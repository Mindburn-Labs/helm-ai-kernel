...
func TestGuardian_SignDecision_TemporalInterventions(t *testing.T) {
	...
	t.Run("Intervention: Interrupt", func(t *testing.T) {
		decision := &contracts.DecisionRecord{ID: "dec-1", SubjectID: "test:subject", Action: "TEST_ACTION", Resource: "test:resource"}
	...
	t.Run("Intervention: Quarantine", func(t *testing.T) {
		decision := &contracts.DecisionRecord{ID: "dec-2", SubjectID: "test:subject", Action: "TEST_ACTION", Resource: "test:resource"}
		intervention := &contracts.InterventionMetadata{
	...
	t.Run("Intervention: None (Fallthrough to PRG)", func(t *testing.T) {
		decision := &contracts.DecisionRecord{ID: "dec-3", SubjectID: "test:subject", Action: "TEST_ACTION", Resource: "test:resource"}
	...
	t.Run("Determinism: Same Input -> Identical Intervention", func(t *testing.T) {
		...
		// Run 1
		d1 := &contracts.DecisionRecord{ID: "run-1", SubjectID: "test:subject", Action: "TEST_ACTION", Resource: "test:resource"}
		_ = g.SignDecision(ctx, d1, effect, []string{}, interventionData)

		// Run 2
		d2 := &contracts.DecisionRecord{ID: "run-2", SubjectID: "test:subject", Action: "TEST_ACTION", Resource: "test:resource"}
