...
func (c *RekorClient) searchByHash(packHash string) (*RekorEntry, error) {
	endpoint := strings.TrimRight(c.logURL, "/") + "/api/v1/index/retrieve"
	payload, err := json.Marshal(map[string]string{"hash": packHash})
	if err != nil {
		return nil, fmt.Errorf("failed to encode rekor search request: %w", err)
	}

	req, err := http.NewRequest(http.MethodPost, endpoint, bytes.NewReader(payload))
	if err != nil {
		return nil, fmt.Errorf("failed to create rekor search request: %w", err)
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return nil, fmt.Errorf("rekor search returned %s: %s", resp.Status, strings.TrimSpace(string(body)))
	}
	...
	return c.fetchEntry(ids[0])
}

func (c *RekorClient) fetchEntry(entryID string) (*RekorEntry, error) {
	endpoint := strings.TrimRight(c.logURL, "/") + "/api/v1/log/entries/" + entryID
	resp, err := c.httpClient.Get(endpoint)
	if err != nil {
		return nil, fmt.Errorf("rekor entry request failed: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		body, _ := io.ReadAll(io.LimitReader(resp.Body, 4096))
		return nil, fmt.Errorf("rekor entry fetch returned %s: %s", resp.Status, strings.TrimSpace(string(body)))
	}

	var byID map[string]RekorEntry
	...
	return nil, fmt.Errorf("rekor entry response did not contain %s", entryID)
}
