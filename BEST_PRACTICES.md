...
- **No unpatched vulnerabilities of medium or higher severity for over 60 days**
  — tracked through `release/vex/policies.yaml` and OpenVEX source generated by
  `scripts/release/generate_vex.sh`; only describe OpenVEX as a public release
  asset when a GitHub release attaches the generated file.
...
- **Formal methods** — Lean proofs in `proofs/Lean` and
  `proofs/EffectPermitSoundness.lean`, checked by the `lean` workflow. The TLA+
  specs were retired in HELM-756: none was tied to the code it described.
...
- **Secure delivery against MITM** — release assets include checksums, SBOM,
  ...
  `v0.8.4` release attaches those files.
