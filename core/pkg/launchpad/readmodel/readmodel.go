...
func EventsFromRun(run session.LaunchRun) []RunEvent {
	...
	if len(run.MCPRefs) > 0 {
		mcpVerdict = "ALLOW"
		mcpReason = ""
		mcpSummary = "MCP quarantine policy was enforced; unknown tools remain unavailable until a credential-verified durable dispatch admission is bound."
	}
	events := []RunEvent{
...
func MCPThreatReviews(catalog *registry.Catalog, runs []session.LaunchRun) []MCPThreatReview {
	...
	for _, app := range catalog.Apps {
		...
		reviews = append(reviews, MCPThreatReview{
			...
			LastDispatchReceipt: "",
			ProofStatus:         proof,
			Summary:             "No MCP tool dispatch is allowed until server identity, tool names, risk class, expiration, revocation semantics, and a credential-verified durable dispatch admission are bound.",
			FixActions: []FixAction{{
				Label:       "Review MCP tools",
				CLI:         "helm-ai-kernel mcp quarantine",
				Description: "Inspect quarantined MCP servers. Opaque local approval is rejected; dispatch requires a credential-verified durable dispatch admission.",
			}},
			CLIEquivalent: "helm-ai-kernel mcp quarantine",
		})
	}
	return reviews
}
...
func mcpQuarantineState(compiled plan.LaunchPlan, run *session.LaunchRun) (string, string, string) {
	if len(runRefs(run, "mcp")) > 0 {
		return "ALLOW", "", "MCP quarantine policy was enforced; unknown tools remain unavailable until a credential-verified durable dispatch admission is bound."
	}
...
func fixActionsFor(reason, id string) []FixAction {
	...
	case "ERR_MCP_QUARANTINE_UNPROVEN", "ERR_MCP_SERVER_QUARANTINED":
		return []FixAction{{Label: "Review MCP threat", CLI: "helm-ai-kernel mcp quarantine", Description: "Keep unknown tools quarantined. Opaque local approval is rejected; dispatch requires a credential-verified durable dispatch admission."}}
	}
