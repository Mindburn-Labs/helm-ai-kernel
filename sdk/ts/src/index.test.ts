...
describe("HelmClient", () => {
  ...
  describe("error handling", () => {
    ...
    it("populates error fields from response body", async () => {
      fetchSpy.mockResolvedValue(
        errorResponse(422, "DENY_SCHEMA_MISMATCH", "bad schema"),
      );
      const client = new HelmClient({ baseUrl: "http://h" });
      try {
        await client.health();
  ...
  describe("execution boundary surfaces", () => {
    ...
    it("reports approval verification unavailable after creating a challenge", async () => {
      ...
      const challenge = await client.createApprovalWebAuthnChallenge("ap1", {
        actor: "user:alice",
      });
      await expect(
        client.assertApprovalWebAuthnChallenge("ap1", {
          challenge_id: "ch1",
    ...
    it("does not submit generic approval transitions without verification", async () => {
      const client = new HelmClient({ baseUrl: "http://h" });
      await expect(
        client.transitionApprovalCeremony("ap1", "approve", {
          reason: "reviewed",
        }),
      ).rejects.toMatchObject({
        status: 503,
    ...
    it("lists negative conformance vectors", async () => {
      fetchSpy.mockResolvedValue(
        jsonResponse([{ id: "pdp-outage", category: "policy" }]),
      );
      const client = new HelmClient({ baseUrl: "http://h" });
    ...
    it("manages MCP quarantine records", async () => {
      ...
      const client = new HelmClient({ baseUrl: "http://h" });
      const result = await client.discoverMcpServer({
        server_id: "mcp1",
        risk: "high",
      });
      expect(fetchSpy).toHaveBeenCalledWith(
    ...
    it("inspects sandbox grants with query parameters", async () => {
      ...
      const client = new HelmClient({ baseUrl: "http://h" });
      const result = await client.inspectSandboxGrants(
        "wazero",
        "deny-default",
        "epoch1",
      );
