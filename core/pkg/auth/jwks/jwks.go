// Package jwks verifies bearer JWTs against an issuer's JSON Web Key Set.
//
// It is the one token verifier (R4): the MCP gateway's OAuth bearer tokens,
// the ADR-0005 Control Plane identity tokens on the kernel, and the effect
// gateway's tokens all go through JWKSValidator. It imports nothing from the
// kernel, so both binaries and the effect gateway can depend on it.
package jwks
...
// OAuthTokenClaims contains the validated claims callers authorize on.
type OAuthTokenClaims struct {
	...
	// AuthorizationDetails is the RFC 9396 "authorization_details" claim as
...
// normalized OAuth metadata used by scope and resource policy.
