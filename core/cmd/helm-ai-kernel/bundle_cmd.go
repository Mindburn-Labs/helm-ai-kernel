...
// runBundleCmd implements `helm-ai-kernel bundle <list|verify|inspect|build>`.
func runBundleCmd(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		_, _ = fmt.Fprintln(stderr, "Usage: helm-ai-kernel bundle <list|verify|inspect|build> [flags]")
...
func runBundleBuild(args []string, stdout, stderr io.Writer) int {
	...
	if cmd.NArg() < 1 {
		...
		_, _ = fmt.Fprintln(stderr, "Usage: helm-ai-kernel bundle build [--language=cel|rego|cedar] [--entities=path] <source>")
		return 2
	}
	srcPath := cmd.Arg(0)
