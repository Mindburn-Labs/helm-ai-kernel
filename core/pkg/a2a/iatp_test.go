...
// stubVerifier returns a verifier function that uses Ed25519 verification.
func stubVerifier() func(pubKey, data, sig string) bool {
...
func TestIATPAuthenticator_ChallengeResponse(t *testing.T) {
	...
	session, err := authA.VerifyResponse(challenge, response, stubVerifier())
	if err != nil {
		t.Fatal(err)
	}
...
func TestIATPAuthenticator_ReplayProtection(t *testing.T) {
	...
	_, err = authA.VerifyResponse(challenge, response, stubVerifier())
	if err != nil {
		t.Fatal(err)
	}
