...
plan. Companion to the TLA+ spec `proofs/GuardianPipeline.tla`, which models
the gate pipeline's ordering properties (the other TLA+ specs were retired in
HELM-756); the Lean proof targets the single safety property that admits a
clean function-level statement.
...
4. **Concurrency / re-entrancy.** `Decide` is modeled as a pure
   ...
   ordering guarantees are covered by the TLA+ spec `GuardianPipeline.tla`
   (model-checked by the `tla` workflow), not here.
