...
func (s *SQLiteReceiptStore) Get(ctx context.Context, decisionID string) (*contracts.Receipt, error) {
	query := `
	        SELECT receipt_id, decision_id, effect_id, external_reference_id, status, blob_hash, output_hash, timestamp, executor_id, metadata, signature, merkle_root, prev_hash, lamport_clock, args_hash, log_id, leaf_index, transparency, receipt_envelope
        FROM receipts
        WHERE decision_id = ?
    `
...
func insertSQLiteReceipt(ctx context.Context, execer sqlExecer, r *contracts.Receipt) error {
	...
	receiptEnvelope, err := json.Marshal(r)
	if err != nil {
	...
	_, err = execer.ExecContext(ctx, query,
		r.ReceiptID, r.DecisionID, r.EffectID, r.ExternalReferenceID, r.Status, r.BlobHash, r.OutputHash, timestamp, r.ExecutorID, string(metaJSON), r.Signature, r.MerkleRoot, r.PrevHash, r.LamportClock, r.ArgsHash, r.LogID, r.LeafIndex, nullableJSON(transparencyJSON), string(receiptEnvelope),
	)
	if err != nil {
...
func (s *SQLiteReceiptStore) queryOne(ctx context.Context, query string, arg any) (*contracts.Receipt, error) {
	...
	err := row.Scan(&receiptID, &decisionID, &effectID, &externalID, &status, &blobHash, &outputHash, &timestamp, &executorID, &metaJSON, &signature, &merkleRoot, &prevHash, &lamport, &argsHash, &logID, &leafIndex, &transparency, &receiptEnvelope)
	if err != nil {
		if err == sql.ErrNoRows {
	...
	return receiptFromSQLiteFieldsWithEnvelope(receiptID, decisionID, effectID, externalID, status, blobHash, outputHash, timestamp, executorID, metaJSON, signature, merkleRoot, prevHash, lamport, argsHash, logID, leafIndex, transparency, receiptEnvelope)
}
...
func scanSQLiteReceipt(scanner sqliteScanner) (*contracts.Receipt, error) {
	...
	if err := scanner.Scan(&receiptID, &decisionID, &effectID, &externalID, &status, &blobHash, &outputHash, &timestamp, &executorID, &metaJSON, &signature, &merkleRoot, &prevHash, &lamport, &argsHash, &logID, &leafIndex, &transparency, &receiptEnvelope); err != nil {
		return nil, err
	}
	return receiptFromSQLiteFieldsWithEnvelope(receiptID, decisionID, effectID, externalID, status, blobHash, outputHash, timestamp, executorID, metaJSON, signature, merkleRoot, prevHash, lamport, argsHash, logID, leafIndex, transparency, receiptEnvelope)
}

func receiptFromSQLiteFieldsWithEnvelope(receiptID, decisionID, effectID string, externalID sql.NullString, status, blobHash, outputHash, timestamp string, executorID, metaJSON, signature, merkleRoot, prevHash sql.NullString, lamport uint64, argsHash sql.NullString, logID sql.NullString, leafIndex uint64, transparency, receiptEnvelope sql.NullString) (*contracts.Receipt, error) {
	if receiptEnvelope.Valid && strings.TrimSpace(receiptEnvelope.String) != "" {
		var receipt contracts.Receipt
		if err := json.Unmarshal([]byte(receiptEnvelope.String), &receipt); err != nil {
			return nil, fmt.Errorf("decode receipt envelope: %w", err)
		}
		if receipt.ReceiptID != receiptID || receipt.DecisionID != decisionID || receipt.EffectID != effectID {
			return nil, fmt.Errorf("receipt envelope identity does not match indexed receipt")
		}
		return &receipt, nil
	}
