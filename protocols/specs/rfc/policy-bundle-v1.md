...
The YAML keys and the JSON keys are not identical. When a bundle crosses a JSON
boundary (`bundle inspect`, `bundle list --json`, or any Go marshalling), the
top-level `apiVersion` is emitted as **`api_version`**. All other keys keep
their spelling.

```json
{
  "api_version": "helm.mindburn.run/v1",
  "kind": "PolicyBundle",
  "metadata": { "name": "corporate-baseline", "version": "1.2.0", "hash": "76d0…" },
  "rules": [
    { "id": "deny-system-writes", "action": "file_write",
      "expression": "params.path.startsWith(\"/etc/\")",
      "verdict": "BLOCK", "reason": "Writing to system directories is prohibited" }
  ]
}
```
...
Multi-language policy compilation (CEL, Rego, Cedar) is a **separate** pipeline
reached through `bundle build`; see §6.3.
...
### 6.3 What `bundle build` actually signs

`bundle build` is implemented, but it does **not** operate on the format in §4
and does not produce a signature file. It compiles a single policy source in
CEL, Rego, or Cedar through the `policybundles` registry and prints the
compiled bundle's identity as JSON:
