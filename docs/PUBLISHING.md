...
Release target: `v0.5.9`. The release is complete only after the tagged
...
AI Act high-risk reference pack. The `v0.5.9` release attaches a
`helm-ai-kernel.rb` formula asset for version `0.5.9` and publishes the same
...
For `v0.5.9`, use `SHA256SUMS.txt`, `sbom.json`, `v0.5.9.openvex.json`,
