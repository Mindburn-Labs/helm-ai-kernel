...
| Go | `go get github.com/Mindburn-Labs/helm-ai-kernel/sdk/go@v0.5.15`; the Go module is released by the subdirectory tag `sdk/go/v0.5.15` | `cd sdk/go && go test ./...` |
| Java | Maven Central coordinate `io.github.mindburnlabs:helm-sdk:0.5.15` | `make test-sdk-java` |
...
Current source release target: `v0.5.15`: <https://github.com/Mindburn-Labs/helm-ai-kernel/releases/tag/v0.5.15>.

The release is complete only after the release page includes Darwin/Linux/Windows binaries, `SHA256SUMS.txt`, `sbom.json`, `v0.5.15.openvex.json`, `release-attestation.json`, `evidence-pack.tar`, `release.high_risk.v3.toml`, `sample-policy-material.tar`, `helm-ai-kernel-launchpad-data.tar`, `helm-console-web-v0.5.15.tar.gz` with checksum/SBOM/provenance/lock/manifest sidecars, `helm-ai-kernel.mcpb`, `helm-ai-kernel.rb`, `v0.5.15.json`, `version-status.json`, and matching `*.cosign.bundle` files for each primary asset.
