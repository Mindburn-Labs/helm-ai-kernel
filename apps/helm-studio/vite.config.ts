...
export default defineConfig({
  ...
  base: studioBase,
  server: {
    // OSS scope: allow the Studio app root + the shared `packages/` workspace
    // (for `@helm/design-tokens` CSS imports). No `modules/*` entry because
    // helm-oss has no `modules/` tree — private modules live only in the
    // commercial repo and are loaded there via an overlayed main.tsx.
    fs: {
      allow: [
