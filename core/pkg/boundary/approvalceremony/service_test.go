...
func TestServiceFailsClosedBeforePersistingUntrustedOrUnrandomizedHold(t *testing.T) {
	...
	t.Run("binding scope substitution", func(t *testing.T) {
		...
		if _, err := service.BeginHold(context.Background(), requestForSpec(hold.Spec)); !errors.Is(err, ErrBindingUnavailable) {
	...
	t.Run("authority metadata substitution", func(t *testing.T) {
		...
		if _, err := service.BeginHold(context.Background(), requestForSpec(hold.Spec)); !errors.Is(err, ErrAuthorityUnavailable) {
	...
	t.Run("randomness unavailable", func(t *testing.T) {
		...
		if _, err := service.BeginHold(context.Background(), requestForSpec(hold.Spec)); !errors.Is(err, io.ErrUnexpectedEOF) {
...
func TestServiceRejectsExpiredHoldBeforeAuthorityOrRandomness(t *testing.T) {
	...
	if _, err := service.IssueChallenge(context.Background(), hold.TenantID, hold.ApprovalID); !errors.Is(err, ErrTransitionConflict) {
...
func TestPostgresStoreRejectsCallerFabricatedTransitionTimes(t *testing.T) {
	...
	if _, err := store.issueChallenge(ctx, hold.TenantID, hold.ApprovalID, challenge, challenge.IssuedAt.Add(time.Second)); !errors.Is(err, ErrInvalidRecord) {
		t.Fatalf("issueChallenge() error = %v, want ErrInvalidRecord", err)
	}
	if _, err := store.recordQuorum(ctx, hold.TenantID, hold.ApprovalID, verified, verified.VerifiedAt.Add(time.Second)); !errors.Is(err, ErrInvalidRecord) {
		t.Fatalf("recordQuorum() error = %v, want ErrInvalidRecord", err)
	}
	if _, err := store.issueGrant(
		ctx, hold.TenantID, hold.ApprovalID, grant,
...
func (s *serviceTestStore) Get(context.Context, string, string) (Record, error) {
...
func (s *serviceTestStore) issueChallenge(context.Context, string, string, contracts.ApprovalChallenge, time.Time) (Record, error) {
	s.issueChallengeCalls++
	return Record{}, errors.New("unexpected issueChallenge call")
}

func (*serviceTestStore) recordQuorum(context.Context, string, string, approvalverify.VerifiedApprovalRef, time.Time) (Record, error) {
	return Record{}, errors.New("unexpected recordQuorum call")
}

func (*serviceTestStore) issueGrant(context.Context, string, string, contracts.ApprovalGrant, string, string, time.Time) (Record, error) {
	return Record{}, errors.New("unexpected issueGrant call")
}

func (s *serviceTestStore) consumeGrant(context.Context, string, string, string, string, string, string, string, time.Time) (Record, error) {
	s.consumeCalls++
	return Record{}, errors.New("unexpected consumeGrant call")
}

func (*serviceTestStore) deny(context.Context, string, string, time.Time) (Record, error) {
	return Record{}, errors.New("unexpected deny call")
}

func (*serviceTestStore) expire(context.Context, string, string, time.Time) (Record, error) {
...
func requestForSpec(spec ChallengeSpec) HoldRequest {
	return HoldRequest{TenantID: spec.TenantID, WorkspaceID: spec.WorkspaceID, BindingRef: spec.BindingRef}
}

func consumerForSpec(spec ChallengeSpec) ConsumerIdentity {
	return ConsumerIdentity{
		Subject: "spiffe://helm/data-plane-a", TenantID: spec.TenantID, WorkspaceID: spec.WorkspaceID,
	}
}
