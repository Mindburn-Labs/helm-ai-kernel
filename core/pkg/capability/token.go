...
// ValidateShape enforces the capability-token/v1 structural rules. Signature
// and lifecycle checks happen in the verifier, not here.
func (t *Token) ValidateShape() error {
	...
	if t.CapabilityRef.ManifestHash == "" {
		return fmt.Errorf("capability_ref.manifest_hash is required (manifest revision must be pinned)")
	}
...
// DecodeToken tolerantly decodes a token from a context value: a JSON string
// or an already-decoded map (e.g. from a JSON request body).
