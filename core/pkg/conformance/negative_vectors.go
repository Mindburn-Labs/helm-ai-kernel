...
func DefaultNegativeBoundaryVectors() []NegativeBoundaryVector {
	return []NegativeBoundaryVector{
		...
		{
			ID:                 "tinyfish-web-missing-api-key",
			Category:           "governed-web",
			Trigger:            "TinyFish Search, Fetch, Browser, or Agent request has no governed X-API-Key credential",
			ExpectedVerdict:    contracts.VerdictDeny,
			ExpectedReasonCode: contracts.ReasonIdentityIsolationViolation,
			MustEmitReceipt:    true,
			MustNotDispatch:    true,
			MustBindEvidence:   []string{"connector_contract_hash", "policy_hash", "request_hash", "credential_ref"},
		},
		{
			ID:                 "tinyfish-search-fetch-source-hash-missing",
			Category:           "governed-web",
			Trigger:            "TinyFish Search or Fetch result is proposed for evidence without source URL hashes",
			ExpectedVerdict:    contracts.VerdictDeny,
			ExpectedReasonCode: contracts.ReasonVerification,
			MustEmitReceipt:    true,
			MustNotDispatch:    true,
			MustBindEvidence:   []string{"connector_contract_hash", "policy_hash", "request_hash", "response_hash", "source_url_hashes"},
		},
		{
			ID:                 "tinyfish-fetch-partial-source-error",
			Category:           "governed-web",
			Trigger:            "TinyFish Fetch returns partial source errors and the workflow tries to promote the result as complete",
			ExpectedVerdict:    contracts.VerdictDeny,
			ExpectedReasonCode: contracts.ReasonVerification,
			MustEmitReceipt:    true,
			MustNotDispatch:    true,
			MustBindEvidence:   []string{"connector_contract_hash", "request_hash", "response_hash", "error_hash", "source_url_hashes"},
		},
		{
			ID:                 "tinyfish-browser-credential-grant-escalation",
			Category:           "governed-web",
			Trigger:            "TinyFish Browser session requests credential use without a credential grant, TTL, and owner approval",
			ExpectedVerdict:    contracts.VerdictEscalate,
			ExpectedReasonCode: contracts.ReasonApprovalRequired,
			MustEmitReceipt:    true,
			MustNotDispatch:    true,
			MustBindEvidence:   []string{"connector_contract_hash", "policy_hash", "credential_grant_ref", "ttl_seconds", "approval_receipt_id"},
		},
		{
			ID:                 "tinyfish-agent-side-effect-approval",
			Category:           "governed-web",
			Trigger:            "TinyFish Agent goal can submit, purchase, send, publish, or otherwise affect external state without approval",
			ExpectedVerdict:    contracts.VerdictEscalate,
			ExpectedReasonCode: contracts.ReasonApprovalRequired,
			MustEmitReceipt:    true,
			MustNotDispatch:    true,
			MustBindEvidence:   []string{"connector_contract_hash", "policy_hash", "request_hash", "action_intent_hash", "approval_receipt_id"},
		},
		{
			ID:                 "tinyfish-web-schema-drift",
			Category:           "governed-web",
			Trigger:            "TinyFish endpoint family or response shape differs from the pinned connector contract",
			ExpectedVerdict:    contracts.VerdictDeny,
			ExpectedReasonCode: contracts.ReasonSchemaViolation,
			MustEmitReceipt:    true,
			MustNotDispatch:    true,
			MustBindEvidence:   []string{"connector_contract_hash", "pinned_schema_hash", "response_shape_hash", "endpoint_family_hash"},
		},
		{
			ID:                 "tinyfish-sample-fixture-production-rejection",
			Category:           "governed-web",
			Trigger:            "sample_only TinyFish fixture artifact is promoted into a production receipt or EvidencePack",
			ExpectedVerdict:    contracts.VerdictDeny,
			ExpectedReasonCode: contracts.ReasonProvenance,
			MustEmitReceipt:    true,
			MustNotDispatch:    true,
			MustBindEvidence:   []string{"fixture_id", "sample_only", "receipt_id", "evidence_pack_ref"},
		},
		{
