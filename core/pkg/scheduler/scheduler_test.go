...
func TestParseRRULE_NotImplemented(t *testing.T) {
	err := ParseRRULE("FREQ=DAILY;INTERVAL=1")
	require.Error(t, err)
	assert.ErrorIs(t, err, ErrRRULENotImplemented)
}
...
func TestValidate_RRULEKind(t *testing.T) {
	...
	err := Validate(s)
	// RRULE is not implemented, so the expression parse should fail.
	require.Error(t, err)
}
...
func TestValidate_AllScheduleKinds(t *testing.T) {
	...
	// RRULE: known kind but expression parse fails (not implemented).
	s2 := validSpec("s2")
	s2.Kind = ScheduleRRULE
	s2.Expression = "FREQ=DAILY"
	require.Error(t, Validate(s2))
}
