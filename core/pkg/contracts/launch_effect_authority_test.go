...
func TestLaunchEffectAuthorizationEnvelopeFailsClosed(t *testing.T) {
	tests := []struct {
		...
		{name: "active scoped stop at dispatch", mutate: func(envelope *contracts.LaunchEffectAuthorizationEnvelope, ctx *contracts.LaunchEffectEnvelopeVerificationContext) {
...
func TestLaunchEffectReceiptSigningRevisionAndStateMachine(t *testing.T) {
	...
	changedWithoutReconciliation.Lamport = 3
...
func TestLaunchEffectReceiptRejectsNoncanonicalAndUntrustedProof(t *testing.T) {
	...
	missingProof.ResolveEvidenceDAG = func(string) (contracts.LaunchEffectEvidenceDAG, error) {
		return contracts.LaunchEffectEvidenceDAG{}, fmt.Errorf("node not found")
	}
	...
	wrongReservation.ResolveAuthority = func(string, string) (contracts.LaunchEffectReceiptAuthorityBinding, error) {
		return contracts.LaunchEffectReceiptAuthorityBinding{}, fmt.Errorf("reservation not found")
	}
	...
	for _, test := range []struct {
		name   string
		mutate func(*contracts.LaunchEffectReceiptAuthorityBinding)
	}{
		{"provider account", func(authority *contracts.LaunchEffectReceiptAuthorityBinding) {
			authority.ProviderAccountRef = "provider-account-other"
		}},
		{"provider action", func(authority *contracts.LaunchEffectReceiptAuthorityBinding) {
			authority.ProviderActionURN = contracts.LaunchProviderActionDigitalOceanTeardown
		}},
		{"provider payload", func(authority *contracts.LaunchEffectReceiptAuthorityBinding) {
			authority.ProviderPayloadHash = launchHash("f")
		}},
	} {
		t.Run("source-owned "+test.name, func(t *testing.T) {
			wrongAuthority := launchReceiptVerificationContext(publicKey)
			resolveAuthority := wrongAuthority.ResolveAuthority
			wrongAuthority.ResolveAuthority = func(ref, hash string) (contracts.LaunchEffectReceiptAuthorityBinding, error) {
				authority, resolveErr := resolveAuthority(ref, hash)
				test.mutate(&authority)
				return authority, resolveErr
			}
			if err := contracts.VerifyLaunchEffectReceipt(signed, wrongAuthority); err == nil {
				t.Fatalf("receipt accepted a %s absent from source-owned authority", test.name)
			}
		})
	}
	...
	cyclicProof.ResolveEvidenceDAG = func(nodeHash string) (contracts.LaunchEffectEvidenceDAG, error) {
		...
		return contracts.LaunchEffectEvidenceDAG{Nodes: []contracts.LaunchEffectEvidenceNode{
			{NodeHash: nodeHash, ParentHashes: []string{otherHash}, ProofSessionRef: signed.ProofSessionRef, EvidenceReservationRef: signed.EvidenceReservationRef, Lamport: 1},
			{NodeHash: otherHash, ParentHashes: []string{nodeHash}, ProofSessionRef: signed.ProofSessionRef, EvidenceReservationRef: signed.EvidenceReservationRef, Lamport: 1},
		}}, nil
	}
	if err := contracts.VerifyLaunchEffectReceipt(signed, cyclicProof); err == nil {
		t.Fatal("receipt accepted a cyclic evidence graph")
	}

	terminalAtRevisionOne := launchUnknownReceiptFixture()
	terminalAtRevisionOne.Outcome = "SUCCEEDED"
	terminalAtRevisionOne.ReconciliationStatus = "PROVEN_APPLIED"
	terminalAtRevisionOne.ReconciliationRevision = 1
	terminalAtRevisionOne.DependencyState = "RELEASED"
	terminalAtRevisionOne.ProviderOperationRef = "provider-operation-1"
	terminalAtRevisionOne.ProviderResourceRefs = []string{"do:app:1"}
	terminalAtRevisionOne.EvidencePackRef = "evidencepack:invalid-cycle"
	terminalAtRevisionOne.EvidencePackHash = launchHash("e")
	if _, err := contracts.SignLaunchEffectReceipt(terminalAtRevisionOne, privateKey); err == nil {
		t.Fatal("terminal receipt revision one created a circular EvidencePack closure")
	}

	terminal := signed
	terminal.ReceiptID = ""
	terminal.Signature = ""
	terminal.ReceiptRevision = 2
	terminal.ReconciliationRevision = 1
	terminal.PreviousReceiptID = signed.ReceiptID
	terminal.Outcome = "SUCCEEDED"
	terminal.ReconciliationStatus = "PROVEN_APPLIED"
	terminal.DependencyState = "RELEASED"
	terminal.ProviderOperationRef = "provider-operation-1"
	terminal.ProviderResourceRefs = []string{"do:app:1"}
	terminal.EvidencePackRef = "evidencepack:terminal"
...
func launchCanonicalApprovalAuthority(t *testing.T, effectID string, input map[string]any, planHash, effectHash string, now time.Time, principal, audience, trustRootID string) (contracts.LaunchEffectApprovalAuthority, string) {
	...
	connectorAuthority := launchTestConnectorAuthority(t, effectID, input, planHash, effectHash, policyHash, now)
...
func launchTestConnectorAuthority(t *testing.T, effectID string, input map[string]any, planHash, effectHash, policyHash string, now time.Time) contracts.ApprovalConnectorAuthority {
	t.Helper()
	...
	authority, err := (contracts.ApprovalConnectorAuthority{
		...
		ConnectorSignatureRef: release.ConnectorSignatureRef, ConnectorSignatureHash: release.ConnectorSignatureHash, ConnectorSignerID: release.ConnectorSignerID,
		ConnectorSandboxProfile: release.ConnectorSandboxProfile, ConnectorDriftPolicyRef: release.ConnectorDriftPolicyRef,
		CertificationRef: release.CertificationRef, CertificationHash: release.CertificationHash, CertificationAuthority: release.CertificationAuthority,
	}).Seal()
	if err != nil {
		t.Fatal(err)
	}
...
func launchMissionReferencePackBytes(t *testing.T) []byte {
	...
	receiptInput.PlanHash = envelope.PlanHash
	receiptInput.RequestHash = envelope.RequestBodyHash
	receiptInput.ArgsC14NHash = envelope.ArgsC14NHash
	...
	receiptInput.ProofSessionRef = envelope.ProofSessionRef
	receiptInput.EvidenceReservationRef = envelope.EvidenceReservationRef
	...
	placement := routeFixture.route.Placements[0]
	receiptInput.RouteBindingRef = routeFixture.route.RouteID
	receiptInput.RouteBindingHash = routeHash
	receiptInput.RoutePlacementID = placement.PlacementID
	receiptInput.ProviderID = placement.ProviderID
	receiptInput.ProviderAccountRef = placement.ProviderAccountRef
	receiptInput.ProviderAccountHash = placement.ProviderAccountHash
	receiptInput.RegionID = placement.RegionID
	receiptInput.OfferingID = placement.OfferingID
	receiptInput.ProviderConnectorID = placement.ProviderConnectorID
	receiptInput.ProviderConnectorContractHash = placement.ProviderConnectorContractHash
	for _, action := range placement.ActionBindings {
		if action.EffectID == receiptInput.EffectID {
			receiptInput.ProviderActionURN = action.ProviderActionURN
			receiptInput.ProviderPayloadHash = action.ProviderPayloadHash
			break
		}
	}
	receiptInput.ProviderProfileRef = placement.ProviderProfileRef
	receiptInput.ProviderProfileHash = placement.ProviderProfileHash
	receiptInput.ProviderCertificationRef = placement.ProviderCertificationRef
	receiptInput.ProviderCertificationHash = placement.ProviderCertificationHash
	...
	pack := map[string]any{
		...
		"receipt": map[string]any{
			...
			"authority_binding": receiptAuthority, "evidence_dag": receiptEvidence,
		},
...
func launchUnknownReceiptFixture() contracts.LaunchEffectReceipt {
	return contracts.LaunchEffectReceipt{
		...
		PlanHash:                      launchHash("0"),
		RequestHash:                   launchHash("3"),
		ArgsC14NHash:                  launchHash("1"),
		...
		ProviderAccountHash:           launchHash("a"),
		RegionID:                      "fra",
		OfferingID:                    "app-platform",
		ProviderConnectorID:           contracts.LaunchConnectorDigitalOcean,
		ProviderConnectorContractHash: launchHash("6"),
		ProviderActionURN:             contracts.LaunchProviderActionDigitalOceanProvision,
		ProviderPayloadHash:           launchHash("b"),
...
func launchReceiptVerificationContext(publicKey ed25519.PublicKey) contracts.LaunchEffectReceiptVerificationContext {
	...
	return contracts.LaunchEffectReceiptVerificationContext{
		...
		ResolveAuthority: func(reservationRef, reservationHash string) (contracts.LaunchEffectReceiptAuthorityBinding, error) {
			if reservationRef != authority.EffectReservationRef || reservationHash != authority.EffectReservationHash {
				return contracts.LaunchEffectReceiptAuthorityBinding{}, fmt.Errorf("durable effect authority not found")
			}
			return authority, nil
		},
		...
		VerifyEvidencePack: func(evidencePackRef, evidencePackHash, previousReceiptID string) error {
			if !strings.HasPrefix(evidencePackRef, "evidencepack:") || !strings.HasPrefix(evidencePackHash, "sha256:") || len(previousReceiptID) != 64 {
				return fmt.Errorf("EvidencePack does not close the signed predecessor")
			}
			return nil
		},
	}
...
func launchReceiptAuthorityFixture() contracts.LaunchEffectReceiptAuthorityBinding {
	receipt := launchUnknownReceiptFixture()
	return launchReceiptAuthorityBinding(receipt)
}

func launchReceiptAuthorityBinding(receipt contracts.LaunchEffectReceipt) contracts.LaunchEffectReceiptAuthorityBinding {
	return contracts.LaunchEffectReceiptAuthorityBinding{
		EffectReservationRef: receipt.EffectReservationRef, EffectReservationHash: receipt.EffectReservationHash,
		EffectID: receipt.EffectID, TenantID: receipt.TenantID, WorkspaceID: receipt.WorkspaceID, MissionID: receipt.MissionID,
		Principal: receipt.Principal, Audience: receipt.Audience, KernelTrustRootID: receipt.KernelTrustRootID, EffectOrdinal: receipt.EffectOrdinal,
		InputSchemaHash: receipt.InputSchemaHash, InputHash: receipt.InputHash, IdempotencyKey: receipt.IdempotencyKey, PlanHash: receipt.PlanHash,
		RequestHash: receipt.RequestHash, ArgsC14NHash: receipt.ArgsC14NHash, KernelVerdictRef: receipt.KernelVerdictRef, KernelVerdictHash: receipt.KernelVerdictHash,
		...
		ConnectorID: receipt.Tool, ConnectorContractHash: receipt.ConnectorContractHash,
		ConnectorAuthorityRef: receipt.ConnectorAuthorityRef, ConnectorAuthorityHash: receipt.ConnectorAuthorityHash, ActionURN: receipt.Action,
		...
		ProviderConnectorContractHash: receipt.ProviderConnectorContractHash, ProviderActionURN: receipt.ProviderActionURN, ProviderPayloadHash: receipt.ProviderPayloadHash,
