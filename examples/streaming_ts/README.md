# Streaming Governance Example (TypeScript)

Demonstrates token-by-token LLM streaming through HELM's governed proxy using the standard OpenAI TypeScript SDK.

## How It Works

1. Your application uses the standard OpenAI SDK with `baseURL` pointed at the HELM proxy
...
An evidence pack is a content-addressed, tamper-evident archive that
bundles all governance artifacts for a set of AI operations. It contains
signed decision records, receipts, and proof graph nodes, all linked by
cryptographic hashes. Anyone can verify the pack offline using the
JCS-canonical manifest and SHA-256 root hash.
