...
## Source-target v0.8.3 Coordinates

Run a clean registry check before copying these into a managed client estate.
The following four coordinates target source version `0.8.3`; registry availability is not claimed.

Source version claims are tied to the repository `VERSION` (`0.8.3` for this release).
The matching Go subdirectory tag is `sdk/go/v0.8.3`.

```bash
npm install @mindburn/helm-ai-kernel@0.8.3
python -m pip install helm-sdk==0.8.3
go get github.com/Mindburn-Labs/helm-ai-kernel/sdk/go@v0.8.3
```

The matching Java coordinate is `io.github.mindburnlabs:helm-sdk:0.8.3`:
...
Rust source exists under `sdk/rust/`, but no public crate availability is
claimed here. Recheck the target registry before publishing `cargo add` as a
