...
| Java package cannot be fetched from Maven Central | registry freshness or local Maven cache issue | verify `io.github.mindburnlabs:helm-sdk:0.7.0` against repo1 or `version-status.json`; build from `sdk/java` only when testing local source changes |
