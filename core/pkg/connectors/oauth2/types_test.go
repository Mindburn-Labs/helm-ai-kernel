...
func TestTokenRefresher_EnsureValid_RefreshesToken(t *testing.T) {
	store := NewInMemoryTokenStore()
	ctx := context.Background()
	...
	saved, err := store.GetToken(ctx, "test-connector")
	if err != nil {
		t.Fatal(err)
	}
	if saved.AccessToken != "refreshed-token" {
		t.Fatalf("saved AccessToken = %q", saved.AccessToken)
	}
}
