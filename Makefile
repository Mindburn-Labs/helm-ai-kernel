...
# lint-security runs the blocking gosec gate (golangci-lint, gosec only) over
# core and sdk/go against scripts/ci/gosec-allowlist.txt. CI runs it through the
# `gosec` quality gate in the pr, merge and release profiles.
.PHONY: lint-security
lint-security:
	bash scripts/ci/security_gates.sh gosec
