...
Codex, Claude Code, Hermes, and DeepSeek Harness use local setup hooks.
OpenClaw still uses adapter helpers from `helm-agent-integrations`; those
demos are not the Hermes or DeepSeek ship.
...
- [DeepSeek Harness](/integrations/deepseek) installs a Kernel hook file plus a DSH profile `configPath` row.
