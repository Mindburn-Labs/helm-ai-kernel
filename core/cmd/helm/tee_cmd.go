...
import (
	...
	"github.com/Mindburn-Labs/helm-oss/core/pkg/crypto/tee"
)

// runTeeCmd implements `helm tee <status|selftest|verify>`.
//
// The subcommand surface mirrors `helm trust` and `helm did`: a thin
...
func runTeeCmd(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "Usage: helm tee <subcommand> [flags]")
		fmt.Fprintln(stderr, "")
	...
	case "--help", "-h", "help":
		fmt.Fprintln(stdout, "Usage: helm tee <subcommand> [flags]")
...
func runTeeVerify(args []string, stdout, stderr io.Writer) int {
	...
	if platform == "" || nonceHex == "" || cmd.NArg() < 1 {
		fmt.Fprintln(stderr, "Usage: helm tee verify --platform=<...> --nonce=<hex> <quote-file>")
		return 2
	}
