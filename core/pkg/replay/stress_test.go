...
func (s *stubEventSource) GetRunEvents(_ context.Context, runID string) ([]RunEvent, error) {
...
func (s *stubExecutor) ReplayEvent(_ context.Context, event RunEvent) (string, error) {
...
func TestStress_ManifestModeDry(t *testing.T) {
	events := makeEvents(5)
	source := &stubEventSource{events: map[string][]RunEvent{"run-1": events}}
	engine := NewEngine(source, &stubExecutor{failAt: -1})
...
func TestStress_ManifestModeUnknown(t *testing.T) {
	engine := NewEngine(&stubEventSource{}, &stubExecutor{failAt: -1})
...
func TestStress_IntegrityCheckExecutionFailed(t *testing.T) {
	events := makeEvents(5)
	source := &stubEventSource{events: map[string][]RunEvent{"run-1": events}}
	engine := NewEngine(source, &stubExecutor{failAt: 2})
	session, _ := engine.StartReplay(context.Background(), "run-1")
...
func TestStress_ConcurrentReplaySessions(t *testing.T) {
	...
	source := &stubEventSource{events: eventsMap}
	engine := NewEngine(source, &stubExecutor{failAt: -1})
...
func TestStress_EngineGetSession(t *testing.T) {
	source := &stubEventSource{events: map[string][]RunEvent{"r1": makeEvents(3)}}
	engine := NewEngine(source, &stubExecutor{failAt: -1})
...
func TestStress_EngineNoEvents(t *testing.T) {
	source := &stubEventSource{events: map[string][]RunEvent{"r1": {}}}
	engine := NewEngine(source, &stubExecutor{failAt: -1})
...
func TestStress_ManifestDryHashMismatch(t *testing.T) {
	events := makeEvents(3)
	source := &stubEventSource{events: map[string][]RunEvent{"r": events}}
	engine := NewEngine(source, &stubExecutor{failAt: -1})
...
func TestStress_ReplaySessionCompletionFields(t *testing.T) {
	source := &stubEventSource{events: map[string][]RunEvent{"r1": makeEvents(5)}}
	engine := NewEngine(source, &stubExecutor{failAt: -1})
	session, _ := engine.StartReplay(context.Background(), "r1")
...
func TestStress_ReplayEngineWithClock(t *testing.T) {
	...
	source := &stubEventSource{events: map[string][]RunEvent{"r1": makeEvents(2)}}
	engine := NewEngine(source, &stubExecutor{failAt: -1}).WithClock(func() time.Time { return fixed })
	session, _ := engine.StartReplay(context.Background(), "r1")
...
func TestStress_ReplayHarnessRegisterEngine(t *testing.T) {
	h := NewReplayHarness()
	h.RegisterEngine("test", &stubReplayEngine{output: []byte("result")})
	err := h.VerifyReceipt(context.Background(), &contracts.Receipt{
...
type stubReplayEngine struct{ output []byte }

func (s *stubReplayEngine) Replay(_ context.Context, _ *contracts.ReplayScriptRef) ([]byte, error) {
	return s.output, nil
}
