...
import type { SignatureCheck, VerificationResult } from "./types.js";
...
<p style="color:var(--muted);margin-bottom:1.5rem">Generated by <code>@mindburn/helm</code> v3 evidence verifier</p>
...
<div class="check"><span class="${result.signature.pass ? "check-pass" : "check-fail"}">${result.signature.pass ? "✓" : "✗"}</span><span class="check-label">Signature</span><span class="check-detail">${signatureDetail(result.signature)}</span></div>
...
function signatureDetail(signature: SignatureCheck): string {
    if (!signature.pass) {
        return signature.reason ?? "failed";
    }
    if (signature.verified) {
        return `verified by ${signature.trustedKeyID ?? signature.signerID ?? "trusted key"}`;
    }
    if (signature.signerID) {
        return `hash-bound by ${signature.signerID}; ${signature.reason ?? "trusted_key_not_configured"}`;
    }
    return signature.reason ?? "not signed";
}
