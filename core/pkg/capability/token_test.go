...
func TestToken_ValidateShape(t *testing.T) {
	...
	cases := []struct {
		...
		{"missing signature", func(tk *Token) { tk.Signature = "" }},
...
func TestToken_CapabilityBindingMismatch(t *testing.T) {
	authority, verifier := testAuthority(t, nil)
	token, err := authority.Mint(mintReq("task-1"))
	if err != nil {
		t.Fatalf("Mint: %v", err)
	}
	_, err = verifier.Verify(VerifyRequest{
		Presented:    token,
		TaskID:       "task-1",
		CapabilityID: "helm.cap.fs.read",
	})
	assertReject(t, err, TokenRejectCapabilityMismatch)
}

func TestDecodeToken_RejectsUnknownAndTrailingJSON(t *testing.T) {
	authority, _ := testAuthority(t, nil)
	token, err := authority.Mint(mintReq("task-1"))
	if err != nil {
		t.Fatalf("Mint: %v", err)
	...
	withUnknown := strings.Replace(string(raw), "{", "{\"unexpected\":true,", 1)
	if _, err := DecodeToken(withUnknown); err == nil {
		t.Fatal("unknown token field must fail closed")
	}
	if _, err := DecodeToken(string(raw) + "\n{}"); err == nil {
		t.Fatal("trailing token JSON value must fail closed")
	}
}

func TestToken_MintRequiresAuthorityClock(t *testing.T) {
	signer, err := kcrypto.NewEd25519Signer("capability-token-clock-test")
	if err != nil {
		t.Fatalf("signer: %v", err)
	}
	authority := NewTokenAuthority(signer, testRegistry(t), NewInMemoryTokenStore(), nil)
	_, err = authority.Mint(mintReq("task-1"))
	assertReject(t, err, TokenRejectClockUnavailable)
}
