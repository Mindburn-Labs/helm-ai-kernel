...
// Currently supports:
//
//	helm trust eu-list status [--json] [--fixture path] [--offline]
//
...
func runTrustCmd(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "Usage: helm trust <subcommand> [flags]")
		fmt.Fprintln(stderr, "")
	...
	case "--help", "-h", "help":
		fmt.Fprintln(stdout, "Usage: helm trust <subcommand> [flags]")
...
func runTrustEUList(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "Usage: helm trust eu-list <status>")
		return 2
	}
	switch args[0] {
	case "status":
